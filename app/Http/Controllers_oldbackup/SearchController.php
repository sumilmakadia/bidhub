<?php
		  ...
		  class SearchController extends Controller
		  {
					...
					public function directory(Request $request) {
            				...
            				$seachprocess = $request->all();
												 	
							return view('directory.public', compact('pending','claimed_listing', 'directoriesObjects', 'seachprocess', 'request', 'uploaded', 'company_name', 'postal_code', 'trade', 'type', 'miles', 'city', 'state_id', 'state', 'location','latitude','longitude'));
					}
							 ...
							 $trade_esc = $trade_esc."|All";
							  $projects = project::when($title, function($q) use ($title) {
													...
													if($trade != '') {
												 	...
												 	if($city !== null && $latitude !== null && $longitude !== null) {
												 	    $projects = $projects->orderBy(DB::raw('ABS(latitude - '.(float)$latitude.') + ABS(longitude - '.(float)$longitude.')'),'ASC' )->get();
												 	    ...
												 	    $projects = $projects->orderBy('need_bid_by_date',$sort_by)->paginate(20);
												 	}
