    ...
    $userdata   = DB::table('users')->join('profiles','profiles.user_id','=','users.id')->select('users.*', 'profiles.*')->where('profiles.longitude','<>','')->selectRaw("{$haversine} AS distance")->whereRaw("{$haversine} <= ?", [$radius])->get();
		...
		if(empty($id)){ }else{
                ...
                if(count($queryarray) > 0){
            	    ...
            	    $message = 'New Project <strong>'.$request->title.'</strong> is created. Please <a href="'.url('project-room/show/'.$id).'" target="_blank">click here</a> to view this project.';
            	    mail($setmailids, 'New Project created', $message, $headers);
