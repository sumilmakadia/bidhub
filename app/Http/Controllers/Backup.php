    ...
    public function create(Request $request)
    {
			...
			$roles = DB::table('roles')->get();
			  ...
			  if(Auth::user()->role_id == 1 || Auth::user()->id == $profile->user_id){
                ...
                $files = portfolios_profile_photo::where('profile_id', $profile->user_id)->get();
			  ...
			  return view('profiles.create', compact('roles', 'profile','plans','files'));
    }
...
//    	$profile = profile::where('user_id', $id)->first();
       ...
       /* if(Auth::user()->role_id != 1) { 
        ...
        $coukvalues = $id;
        $cookie_name = "viewcounterpage";
        $currentmonth = date('F');
        $curentyear = date('Y');
        $oldyeardatastr = 'old_year_'.$currentmonth;
        $oldyear = date("Y",strtotime("-1 year"));
        $montharray = array();
        
        if(!isset($_COOKIE[$cookie_name])) {
            
            array_push($viewpagearray,$id);
            $finalcomarray = implode(',',$viewpagearray);
            /*setcookie($cookie_name, $finalcomarray, time() + (86400 * 30), "/"); */
            setcookie($cookie_name, $finalcomarray, time() + (1), "/");
            
            $chkviewer = DB::table('help_wanted_view')->where('help_id',$id)->get();
            
            if(count($chkviewer) > 0){
                
                $getvalueyear_of_month = $chkviewer[0]->$oldyeardatastr;
                $dbmontjson = json_decode($chkviewer[0]->json_arraydata,true);
                
                if($getvalueyear_of_month == $curentyear){
                    
                    $oldcounter = $chkviewer[0]->$currentmonth;
                    $newcounter = 1;
                    $finalcounter = ($oldcounter + $newcounter);
                    
                    
                    
                    if(in_array($currentmonth,$dbmontjson)){}else{
                        array_unshift($dbmontjson, $currentmonth); 
                    ...
                    if (($key = array_search($currentmonth, $dbmontjson)) !== false) {
                        unset($dbmontjson[$key]);
                        array_unshift($dbmontjson, $currentmonth);
                 ...
                 $finaljsondata = json_encode($dbmontjson,true);
            
                DB::table('help_wanted_view')->where('help_id',$id)->update([@$currentmonth => $finalcounter,'year'=>$curentyear,'old_year'=>$oldyear,@$oldyeardatastr =>$curentyear,'json_arraydata'=>$finaljsondata]);
                
            }else{
                
                if(in_array($currentmonth,$montharray)){}else{
                        array_push($montharray, $currentmonth); 
                       
                    }
                
                $finaljsondata = json_encode($montharray,true);
                DB::table('help_wanted_view')->insertGetId(['help_id'=>$id,@$currentmonth => 1,'year'=>$curentyear,'old_year'=>$oldyear,@$oldyeardatastr =>$curentyear,'json_arraydata'=>$finaljsondata]);
            }
            ...
            $cookiearray = explode(',',$_COOKIE[$cookie_name]);
            ...
            if(in_array($id,$cookiearray)){
             ...
             $finalcomarray = implode(',',$cookiearray);
             /*setcookie($cookie_name, $finalcomarray, time() + (86400 * 30), "/");*/
             setcookie($cookie_name, $finalcomarray, time() + (1), "/");
            ...
            if(count($chkviewer) > 0){
                ...
                $finaljsondata = json_encode($dbmontjson,true);
                DB::table('help_wanted_view')->where('help_id',$id)->update([@$currentmonth => $finalcounter,'year'=>$curentyear,'old_year'=>$oldyear,@$oldyeardatastr =>$curentyear]);
                ...
                $finaljsondata = json_encode($montharray,true);
                DB::table('help_wanted_view')->insertGetId(['help_id'=>$id,@$currentmonth => 1,'year'=>$curentyear,'old_year'=>$oldyear,@$oldyeardatastr =>$curentyear]);
            }
    ...
    public function viewer($id){
      ...
      if (isset($profile)){
       $chkhelpown = DB::table('profiles')->where('id',$profile->id)->get();
       if(count($chkhelpown) > 0){
           
           $helps = DB::table('help_wanted_view')->join('profiles','profiles.id','=','help_wanted_view.help_id')->where('help_wanted_view.help_id',$id)->get();
           
            return view('profiles.viewer', compact('helps')); 
    ...
    public function update($id, Request $request)
    {
			...
			if($request->hasFile('files')){
			     ...
			     if(count($chkalimg) > 0){
			         ...
			         $maintotal = (count($chkalimg) + count($files));
			         
			        
			         
			         if(count($chkalimg) == 5 || $maintotal > 5){
			             return redirect()->back();
			         }
