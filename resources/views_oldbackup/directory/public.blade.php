								...
								@else
									@foreach($uploaded as $upload)
										<tr>
										    ...
										    <td><a href="/profiles/show/{{ $upload->profile->id }}" style="color:#fb9678; font-weight:bold;">{{ $upload->businessName }}</a></td>
										    @else
        									...
        									@elseif($profile->approved && $upload->approved)
        								    ...
        								    @else
    											<td><button id="{{ $upload->id }}" {{isset($profile->claim) && $profile->claim->approved == 0 ? 'disabled' : '' }}{{ isset($profile->claim) && $profile->claim->approved == 1 ? 'disabled' : '' }} class="btn btn-primary btn-green claim" data-toggle="modal" data-user="{{ Auth::user()->id }}" data-target="#myModalHorizontal">{{ isset($profile->claim) && $profile->claim->approved == 0 ? 'Unclaimed' : 'Claim This Listing' }}</button></td>
									        @endif
										</tr>
									@endforeach
